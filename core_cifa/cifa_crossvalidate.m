function  cifa_crossvalidate(Observation,RunParam,varargin)
...
%                        3. faInit
%                           Use deconv-FA to provide an initialization 
...
rand('state', 1);
permFile = strcat(fileHeader,'_cifa_permutedindices.mat');
if ~exist(permFile)
...
for iDim =1:length(zDimList) % Change to parfor if evaluating multiple latent dim
    ...
    for cvf = 1:numFold
        cifaFile = strcat(fileHeader,sprintf('_%.03d_%d',zDim,cvf),...
            '_cifa_result');
        if cvf == 0
        ...
        [CifaParam,CifaResult, LL,~,CifaInitParam] = cifa(Observation,...
            RunParam(iDim),'maxIter',maxIter,'FixParam',FixParam(iDim),'InitParam',...
            InitParam(iDim),'leaveOneOut',...
            true,'fileHeader',cifaFile,'splitTrainTest',isSplit,...
            'initType',initType);
            parsave(CifaResult,CifaParam,strcat(cifaFile,'_test.mat'),...
                LL,CifaInitParam);
