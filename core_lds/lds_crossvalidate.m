function  lds_crossvalidate(Observation,RunParam,varargin)
...
rand('state', 1);
permFile = strcat(fileHeader,'_lds_permutedindices.mat');
if ~exist(permFile)
...
for iDim =1:length(zDimList) % Change to parfor if evaluating multiple latent dim
    ...
    for cvf = 1:numFold
        ldsFile = strcat(fileHeader,sprintf('_%.03d_%d',zDim,cvf),...
            '_lds_result');
        if cvf == 0
        ...
        [LdsParam,LdsResult, LL,LdsInitParam] = lds(Observation,...
            RunParam(iDim),'maxIter',maxIter,'FixParam',FixParam,'InitParam',...
            InitParam,'leaveoneout',...
            true,'fileHeader',ldsFile,'splitTrainTest',isSplit,...
            'initType','faInit');
        

            parsave(LdsResult,LdsParam,strcat(ldsFile,'_test.mat'),...
                LL,LdsInitParam);
